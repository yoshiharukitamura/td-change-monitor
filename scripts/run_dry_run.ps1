...
$logFile = Join-Path $logDir ("td_change_monitor_dry_run_" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".log")
...
try {
    ...
    if (-not $hasLock) {
        ...
        & $UvPath run td-change-monitor --dry-run 2>&1 | Tee-Object -FilePath $logFile
