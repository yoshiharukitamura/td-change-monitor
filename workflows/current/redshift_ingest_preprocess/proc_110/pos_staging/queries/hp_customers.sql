...
SELECT
  ...
  , password
  , activation_key
  , TD_TIME_FORMAT(TD_TIME_PARSE(activation_expiration, 'JST'), 'yyyy-MM-dd HH:mm:ss', 'JST') AS activation_expiration
  , activation_sms_key
  , TD_TIME_FORMAT(TD_TIME_PARSE(activation_sms_expiration, 'JST'), 'yyyy-MM-dd HH:mm:ss', 'JST') AS activation_sms_expiration
  , activated
  , activated_from
  , reset_password_date
  , reset_password_key
  ...
  , deleted_date
FROM
  l0_pos_staging.hp_customers AS P_02
JOIN
