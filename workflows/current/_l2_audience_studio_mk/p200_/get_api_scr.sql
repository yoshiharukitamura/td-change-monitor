...
--    array_join(array_agg(scr), ', ') as scr_attr
    array_join(array_agg(scr ORDER BY no), ', ') as scr_attr
  from (
      select
...
, bhvr_tmp as (
  select
    ...
    , table_name
    , table_name_jp
--    , array_join(array_agg('{"name": "'||column_name_jp||'", "'||data_type||'": "string", "parentColumn": "'||column_name||'"}'), ', ') as scr
--    , array_join(array_agg('{"name": "'||column_name_jp|| '", "type": "' || data_type || '", "parentColumn": "'||column_name||'"}'), ', ') as scr
    , array_join(array_agg('{"name": "'||column_name_jp|| '", "type": "' || data_type || '", "parentColumn": "'||column_name||'"}' ORDER BY no
      ), ', ') as scr
  from
    ${database_name}.${table_name}
  where
