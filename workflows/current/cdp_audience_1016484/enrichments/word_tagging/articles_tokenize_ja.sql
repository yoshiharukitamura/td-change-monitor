...
-- CDP: Audience: Word Tagging: audience/enrichments/word_tagging/articles_tokenize_ja.sql
-- console link: ${link}
...
WHERE
  length(word) >= 2
  AND word RLIKE '^[ぁ-んーァ-ヶー一-龠a-zA-Zａ-ｚＡ-Ｚ・！？]+$' -- acceptable characters
  AND word NOT RLIKE '^([^一-龠]{1,2}|[ぁ-んー]{1,3})$' -- even if word consists of acceptable characters, reject "len-2 non-kanji word" and "len-3 hiragana-only word"
