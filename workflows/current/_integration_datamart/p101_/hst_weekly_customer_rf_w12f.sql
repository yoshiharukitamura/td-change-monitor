select
  ...
  , min(if(td_date_trunc('week', time, 'jst') < ymd_unixtime, time, null)) as first_order_unixtime
  ...
  , count(distinct if(time between ymd_unixtime - 60*60*24*7*12 and ymd_unixtime - 1, business_date, null)) as f
  , sum(if(order_id_hour_seq = 1 and time between ymd_unixtime - 60*60*24*7*12 and ymd_unixtime - 1, uriage1)) as sales_amount
  , sum(if(order_id_hour_seq = 1 and time between ymd_unixtime - 60*60*24*7*12 and ymd_unixtime - 1, treatment_minutes_in_hour)) as treatment_minutes
...
from
  ...
  cross join unnest (
    sequence(td_date_trunc('week', td_time_parse('${td.each.date_from}', 'jst'), 'jst'), td_time_parse('${td.each.date_to}', 'jst') , 60*60*24*7)
  ) as t(ymd_unixtime)
  left join (select property_id, cast(shop_no as bigint) as shop_no from _integration_datamart.mst_shop) using (property_id)
where
  customer_id is not null
  and td_date_trunc('week', time, 'jst') < ymd_unixtime
group by
  customer_id
  , ymd_unixtime