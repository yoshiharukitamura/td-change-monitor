with shop_master as (
  ...
  from
    ...
    left join l0_pos.mst_prefectures as t2 on t1.pref_id = t2.id
    left join l0_pos.mst_areas as t3 on t1.mst_area_id = t3.id
)
select
  t0.*
  ...
  , t2.*
from
  z_lost_opportunity_analytics_raw_by_user as t0
  cross join unnest (
      arry_mst_shop_no
  ) as t1(mst_shop_no)
