with prep_timeslot_multiply as (
  select
    *
  from
...
-- v1
    -- , map_agg(business_hour, 
    --   if(
    --       if(td_time_parse(business_day, 'jst') between td_time_parse('2023-12-23', 'jst') and td_time_parse('2024-01-14', 'jst') and is_manual_fixed = 0
    --           , cast(ceiling(time_slot * 1.2) as bigint)
    --           , time_slot
    --         )>latest_bed_num
    --       , latest_bed_num
    --       , if(td_time_parse(business_day, 'jst') between td_time_parse('2023-12-23', 'jst') and td_time_parse('2024-01-14', 'jst') and is_manual_fixed = 0
    ...
    -- , map_agg(business_hour, 
    --   if(
    --       if(td_time_parse(business_day, 'jst') between td_time_parse(date_from, 'jst') and td_time_parse(date_to, 'jst') and is_manual_fixed = 0
    --           , cast(ceiling(time_slot * multiply) as bigint)
    --           , time_slot
    --         )>latest_bed_num
    --       , latest_bed_num
    --       , if(td_time_parse(business_day, 'jst') between td_time_parse(date_from, 'jst') and td_time_parse(date_to, 'jst') and is_manual_fixed = 0
...
-- v2
    -- , map_agg(business_hour, 
    --   if(
    --       if(td_time_parse(business_day, 'jst') between td_time_parse(date_from, 'jst') and td_time_parse(date_to, 'jst')
    --         and business_hour between hour_from	and hour_to
    --         and is_manual_fixed = 0
    --           -- , cast(ceiling(time_slot * multiply) as bigint)
    --           , cast(ceiling(time_slot * if(regexp_like(business_dow_fixed, '土日祝'), weekend, weekdays)) as bigint)
    --           , time_slot
    --         )>latest_bed_num
    --       , latest_bed_num
    --       , if(td_time_parse(business_day, 'jst') between td_time_parse(date_from, 'jst') and td_time_parse(date_to, 'jst')
...
-- v3
  ...
  from
    ...
    fin_timeslot_raw_vtable_fixed as t1
    left join prep_timeslot_multiply as t2
      on t1.mst_shop_id = t2.property_id
  ...
  where
    ...
    t1.time = td_date_trunc('week', td_scheduled_time(), 'jst')
  group by
    1,2,3
)
...
select
  ...
  , kv_ts[9] as dvn3_data_h06
  , kv_ts[9] as dvn3_data_h07
  , kv_ts[9] as dvn3_data_h08
  , kv_td3[9] as dvn3_data_h09
  , kv_td3[10] as dvn3_data_h10
  , kv_td3[11] as dvn3_data_h11
  , kv_td3[12] as dvn3_data_h12
  ...
  , kv_td3[22] as dvn3_data_h22
  , kv_td3[23] as dvn3_data_h23
  , kv_ts[23] as dvn3_data_h24
  , kv_ts[23] as dvn3_data_h25
  , kv_ts[23] as dvn3_data_h26
  , kv_ts[23] as dvn3_data_h27
  , kv_ts[23] as dvn3_data_h28
  , kv_ts[23] as dvn3_data_h29
