with customer_rf_shop as (
  ...
  where
    td_time_range(time, td_time_add(td_date_trunc('week', td_scheduled_time(), 'jst'), '-24w'), td_date_trunc('day', td_scheduled_time(), 'jst'), 'jst')
)
, app_session_master as (
  ...
  from (
      ...
      where td_time_range(time, td_time_add(td_date_trunc('week', td_scheduled_time(), 'jst'), '-24w'), td_date_trunc('day', td_scheduled_time(), 'jst'), 'jst')
    )
    left join (
        ...
        where customer_id is not null
              and td_time_range(time, td_time_add(td_date_trunc('week', td_scheduled_time(), 'jst'), '-24w'), td_date_trunc('day', td_scheduled_time(), 'jst'), 'jst')
        group by 1,2
...
, id_map_by_reservation_nw as (
  select
    user_pseudo_id
    , td_time_string(td_time_add(td_date_trunc('week', event_unixtime, 'jst'), '-7d', 'jst'), 'd!','jst') as week
    , max_by(customer_id, event_unixtime) as customer_id
    , 1 as nw_reserve
  from
    _integration_datamart.z_tmp_cls_app_reserve
...
select
  ...
  , case
      ...
      when td_time_parse(t2.week, 'jst') <= td_date_trunc('week', coalesce(t4.first_order_unixtime, td_scheduled_time()), 'jst') then '新規'
      when td_time_parse(t2.week, 'jst') > td_date_trunc('week', coalesce(t4.first_order_unixtime, td_scheduled_time()), 'jst') then '既存'
      else '例外'
    end as new_repeat
  , case
...
from
  ...
  left join app_session_nw as t01 
    on t0.user_pseudo_id = t01.user_pseudo_id
      and td_date_trunc('week', t0.event_unixtime, 'jst') = td_time_parse(t01.week, 'jst')
  ...
  left join id_map_by_reservation_nw as t31
    on t0.user_pseudo_id = t31.user_pseudo_id
      and td_date_trunc('week', t0.event_unixtime, 'jst') = td_time_parse(t31.week, 'jst')
...
where
  td_time_range(t0.time, td_time_add(td_date_trunc('week', td_scheduled_time(), 'jst'), '-24w'), td_date_trunc('day', td_scheduled_time(), 'jst'), 'jst')
group by
  -- 1,2,3,4,11,12,13
  1,2,3,4,5
order by
