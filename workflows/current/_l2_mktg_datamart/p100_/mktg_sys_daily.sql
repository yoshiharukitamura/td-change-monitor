with customer_rf_shop as (
  ...
  from
    ...
    inner join (select time, customer_id from _integration_datamart.z_tmp_cls_reservation_daily) using (time, customer_id)    
  where
...
select
  ...
  , case
      ...
      when td_time_parse(day, 'jst') <= td_date_trunc('day', coalesce(first_order_unixtime, td_scheduled_time()), 'jst') then '新規'
      when td_time_parse(day, 'jst') > td_date_trunc('day', coalesce(first_order_unixtime, td_scheduled_time()), 'jst') then '既存'
      else '例外'
    end as new_repeat
  , case
...
from
  _integration_datamart.z_tmp_cls_reservation_daily
