...
, prep_reservation as (
  ...
  from
    ...
    left join (select id as reservation_person_id, member_card_no from l1_reservation.reservation_persons) using (reservation_person_id)
    left join (select id as customer_id, members_card_no as member_card_no from l1_pos.customers) using (member_card_no)
    left join (select reservation_id as id, resource_timetable_id from l1_reservation.reservation_relations) using (id)
    left join (select id as resource_timetable_id, mst_shop_id as property_id from l1_reservation.resource_timetables) using (resource_timetable_id)
